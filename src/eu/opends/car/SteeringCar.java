...
public class SteeringCar extends Car 
{
	...
	public void update(float tpf)
	{
		...
		else
		{
			...
			float currentFriction = powerTrain.getFrictionCoefficient() * maxFreeWheelBrakeForce * mul;
	...
	{	
		...
		if((lateralDistance < minLateralSafetyDistance) && (forwardDistance > 0) && 
				(forwardDistance < Math.max(0.5f * getCurrentSpeedKmh() * mul, minForwardSafetyDistance)))
			{
				return true;
			}
